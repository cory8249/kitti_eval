...
vector<tDetection>
loadDetections(string file_name, bool &compute_aos, bool &eval_car, bool &eval_pedestrian, bool &eval_cyclist,
               bool &success) {
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det,
                          const vector<tGroundtruth> &dc, const vector<int32_t> &ignored_gt,
                          const vector<int32_t> &ignored_det, bool compute_fp, bool compute_aos = false,
    ...
    for (int32_t i = 0; i < gt.size(); i++) {
        ...
        for (int32_t j = 0; j < det.size(); j++) {
            ...
            } else if (compute_fp && overlap > MIN_OVERLAP[current_class] && valid_detection == NO_DETECTION &&
                       ignored_det[j] == 1) {
                det_idx = j;
                valid_detection = 1;
...
bool eval_class(FILE *fp_det, FILE *fp_ori, CLASSES current_class, const vector<vector<tGroundtruth> > &groundtruth,
                const vector<vector<tDetection> > &detections, bool compute_aos, vector<double> &precision,
                vector<double> &aos, DIFFICULTY difficulty) {
...
void saveAndPlotPlots(string dir_name, string file_name, string obj_type, vector<double> vals[], bool is_aos) {
    ...
    // open file
    fp = fopen((dir_name + "/" + file_name + ".gp").c_str(), "w");
    ...
    for (int32_t j = 0; j < 2; j++) {
        ...
        if (j == 0) {
            fprintf(fp, "set term png size 800,600 font \"Helvetica\" 20\n");
        ...
        fprintf(fp, "\"%s.txt\" using 1:4 title 'Hard' with lines ls 3 lw %d\n", file_name.c_str(), lw);
...
bool eval(string tracking_gt_file, string tracking_det_file, string result_dir) {
    // set some global parameters
    initGlobals();

    // ground truth and result directories
    string gt_dir = "/media/cory/54604BF5604BDBFC/Project/KITTI_Dataset/data_object_det_2/training/det_2/lsvm4_2_car";
    //string tracking_gt_file = CONFIG_TRACKING_GT_FILENAME;
    //string tracking_det_file = CONFIG_TRACKING_DET_FILENAME;
    ...
    cout << "N_TESTIMAGES: " << N_TESTIMAGES << endl;

    bool use_tracking_format = true;
    if(use_tracking_format) {
        groundtruth = load_tracking_gt(tracking_gt_file);
        detections = load_tracking_det(tracking_det_file);
        eval_car = true;
        eval_pedestrian = true;
    }else {
        for (int32_t i = 0; i < N_TESTIMAGES; i++) {
            ...
            string gt_full_path = gt_dir + "/" + file_name;
            string result_full_path = result_dir + "/data/" + file_name;

            ifstream gt_file(gt_full_path.c_str());
            ifstream result_file(result_full_path.c_str());

            cout << "gt_full_path: " << gt_full_path << " "<< gt_file.good() << endl;
            cout << "result_full_path: " << result_full_path << " "<< result_file.good() << endl;

            vector<tGroundtruth> gt = loadGroundtruth(gt_full_path, gt_success);
            vector<tDetection> det = loadDetections(result_full_path, compute_aos, eval_car, eval_pedestrian,
                                                    eval_cyclist,
                                                    det_success);
            ...
            // check for errors
            if (!gt_success) {
                cout << "ERROR: Couldn't read: %s of ground truth. Please write me an email!" << file_name;
                return false;
            }
            if (!det_success) {
                cout << "ERROR: Couldn't read: %s", result_dir + "/data/" + file_name;
                return false;
            }
        }
    }
    ...
    if (eval_car) {
        ...
        if (!eval_class(fp_det, fp_ori, CAR, groundtruth, detections, compute_aos, precision[0], aos[0], EASY)
            ...
            cout << "Car evaluation failed." << endl;
            return false;
        }
        fclose(fp_det);
        saveAndPlotPlots(result_dir, CLASS_NAMES[CAR] + "_detection", CLASS_NAMES[CAR], precision, 0);
        if (compute_aos) {
            saveAndPlotPlots(result_dir, CLASS_NAMES[CAR] + "_orientation", CLASS_NAMES[CAR], aos, 1);
            fclose(fp_ori);
        }
    }
    ...
    if (eval_pedestrian) {
        ...
        vector<double> precision[3], aos[3];
        cout << "eval_pedestrian" << endl;
        if (!eval_class(fp_det, fp_ori, PEDESTRIAN, groundtruth, detections, compute_aos, precision[0], aos[0], EASY)
            || !eval_class(fp_det, fp_ori, PEDESTRIAN, groundtruth, detections, compute_aos, precision[1], aos[1],
                           MODERATE)
            ||
            !eval_class(fp_det, fp_ori, PEDESTRIAN, groundtruth, detections, compute_aos, precision[2], aos[2], HARD)) {
            cout << "Pedestrian evaluation failed.";
            return false;
        }
        fclose(fp_det);
        saveAndPlotPlots(result_dir, CLASS_NAMES[PEDESTRIAN] + "_detection", CLASS_NAMES[PEDESTRIAN], precision, 0);
        if (compute_aos) {
            fclose(fp_ori);
            saveAndPlotPlots(result_dir, CLASS_NAMES[PEDESTRIAN] + "_orientation", CLASS_NAMES[PEDESTRIAN], aos, 1);
        }
    }
    ...
    if (eval_cyclist) {
        ...
        if (!eval_class(fp_det, fp_ori, CYCLIST, groundtruth, detections, compute_aos, precision[0], aos[0], EASY)
            ...
            cout << "Cyclist evaluation failed.";
            return false;
        }
        fclose(fp_det);
        saveAndPlotPlots(result_dir, CLASS_NAMES[CYCLIST] + "_detection", CLASS_NAMES[CYCLIST], precision, 0);
        if (compute_aos) {
            fclose(fp_ori);
            saveAndPlotPlots(result_dir, CLASS_NAMES[CYCLIST] + "_orientation", CLASS_NAMES[CYCLIST], aos, 1);
        }
    }
...
int main(int32_t argc, char *argv[]) {
    if(argc != 4){
        printf("./kitti_eval <gt_filepath> <det_filepath> <result_dir>\n");
        return -1;
    }
    string gt_filepath = argv[1];
    string det_filepath = argv[2];
    string result_dir = argv[3];

    string mkdir_cmd = string("mkdir -p ") + result_dir;
    system(mkdir_cmd.c_str());
    eval(gt_filepath, det_filepath, result_dir);
    return 0;
}
