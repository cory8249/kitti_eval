...
loadDetections(string file_name, bool &compute_aos, bool &eval_car, bool &eval_pedestrian, bool &eval_cyclist,
    ...
    while (!feof(fp)) {
        ...
        if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
            ...
            d.box.type = str;
            // printf("- %s %f %f %f %f\n", d.box.type.c_str(),  d.box.x1,  d.box.y1,  d.box.x2,  d.box.y2);
...
vector<tGroundtruth> loadGroundtruth(string file_name, bool &success) {
    ...
    while (!feof(fp)) {
        ...
        /*if (fscanf(fp, "%s %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
            ...
            // printf("%s %f %f %f %f\n", str,  g.box.x1,  g.box.y1,  g.box.x2,  g.box.y2);
            g.box.type = string(str);
            groundtruth.push_back(g);
        }else */
        if(fscanf(fp, "%s %lf %lf %lf %lf",str, &g.box.x1, &g.box.y1, &g.box.x2, &g.box.y2) == 5){
            long position = ftell(fp);
            if(fscanf(fp, "%lf %d %lf", &g.truncation, &g.occlusion, &g.box.alpha) == 3){
                ;
            }else{
                fseek(fp, position, 0);
            }
            g.box.type = string(str);
            // cout << g.box.type << " " << g.box.x1 << " " << g.box.y1 << " " << g.box.x2 << " " << g.box.y2 << endl;
            groundtruth.push_back(g);
        }
    }
    fclose(fp);
...
bool eval_class(FILE *fp_det, FILE *fp_ori, CLASSES current_class, const vector<vector<tGroundtruth> > &groundtruth,
    ...
    int num_images = groundtruth.size();
...
bool eval(string gt_list, string det_list) {

    // set some global parameters
    initGlobals();

    string result_dir = "result";
    string plot_dir = result_dir;
    ...
    bool compute_aos = false, eval_car = false, eval_pedestrian = false, eval_cyclist = false;

    ifstream val_labels_file(gt_list);
    ifstream det_labels_file(det_list);

    long val_labels_num = std::count(std::istreambuf_iterator<char>(val_labels_file),
                                     std::istreambuf_iterator<char>(), '\n');
    long det_labels_num = std::count(std::istreambuf_iterator<char>(det_labels_file),
                                     std::istreambuf_iterator<char>(), '\n');
    val_labels_file.clear();
    val_labels_file.seekg (0, ios::beg);
    det_labels_file.clear();
    det_labels_file.seekg (0, ios::beg);

    cout << "val_labels_num " << val_labels_num << "  det_labels_num " << det_labels_num << endl;
    assert(val_labels_num == det_labels_num);

    string gt_label_file_path;
    string det_label_file_path;

    // for all images read groundtruth and detections
    cout << "Loading detections..." << endl;
    for (int i = 0; i < val_labels_num; i++) {

        getline(val_labels_file, gt_label_file_path);
        getline(det_labels_file, det_label_file_path);

        if(!val_labels_file.good() or !det_labels_file.good()){
            break;
        }
        ...
        // cout << i << " gt: " << gt_label_file_path << "   det: " << det_label_file_path << endl;

        vector<tGroundtruth> gt = loadGroundtruth(gt_label_file_path, gt_success);
        vector<tDetection> det = loadDetections(det_label_file_path, compute_aos, eval_car, eval_pedestrian, eval_cyclist, det_success);
        ...
        // check for errors
        if (!gt_success) {
            cout << "ERROR: Couldn't read: " << gt_label_file_path << " of ground truth." << endl;
            return false;
        }
        if (!det_success) {
            cout << "ERROR: Couldn't read: " << det_label_file_path << endl;
            return false;
        }
    }
    cout << "  done.";
    ...
    if (eval_pedestrian) {
        ...
        cout << (result_dir + "/stats_" + CLASS_NAMES[PEDESTRIAN] + "_detection.txt") << endl;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        cout << "Usage: ./eval_detection <gt_file_list> <det_file_list>" << endl;
        return 1;
    }

    // read arguments as ground truth and result directories
    string gt_file_list = argv[1];
    string det_file_list = argv[2];

    eval(gt_file_list, det_file_list);
    return 0;
}
